export const ITEM_CARD_DATA = [
  {
    id: "1",
    image:
      "https://images.unsplash.com/photo-1505740420928-5e560c06d30e?w=400&h=300&fit=crop&crop=center",
    alt: "Auralite Graphite Over-Ear Test",
    category: "Headphones",
    title: "Auralite Graphite Over-Ear Test",
    price: "$149.00",
    subtitle:
      "Balanced sound profile with memory foam ear cushions and 30-hour battery life.",
    stockText: "In stock",
    rating: "★★★★★",
    note: "Quick view or add this item to the cart for fast checkout.",
    brand: "Auralite",
  },
  {
    id: "2",
    image:
      "https://images.unsplash.com/photo-1590658268037-6bf12165a8df?w=400&h=300&fit=crop&crop=center",
    alt: "SonicWave Wireless Earbuds",
    category: "Speakers",
    title: "SonicWave Wireless Earbuds",
    price: "$129.99",
    subtitle:
      "True wireless earbuds with active noise cancellation and 8-hour battery life.",
    stockText: "Only 3 left",
    rating: 3,
    note: "Quick view or add this item to the cart for fast checkout.",
    brand: "Auralite",
  },
  {
    id: "3",
    image:
      "https://images.unsplash.com/photo-1484704849700-f032a568e944?w=400&h=300&fit=crop&crop=center",
    alt: "Crescendo IEM Pro",
    category: "Accessories",
    title: "Crescendo IEM Pro",
    price: "$229.00",
    subtitle:
      "Studio-grade in-ear monitors with detachable cable and tunable filters.",
    stockText: "Pre-order",
    rating: 1,
    note: "Quick view or add this item to the cart for fast checkout.",
    brand: "Crescendo",
  },
  {
    id: "4",
    image:
      "https://images.unsplash.com/photo-1583394838336-acd977736f90?w=400&h=300&fit=crop&crop=center",
    alt: "Auralite Graphite Over-Ear 2",
    category: "Headphones",
    title: "Auralite Graphite Over-Ear",
    price: "$159.00",
    subtitle:
      "Comfortable over-ear design with balanced bass and refined treble.",
    stockText: "In stock",
    rating: 3,
    note: "Quick view or add this item to the cart for fast checkout.",
    brand: "Auralite",
  },
  {
    id: "5",
    image:
      "https://images.unsplash.com/photo-1545127398-14699f92334b?w=400&h=300&fit=crop&crop=center",
    alt: "Boreal Pulse Mini Speaker 2",
    category: "Speakers",
    title: "Boreal Pulse Mini Speaker",
    price: "$84.99",
    subtitle:
      "Compact wireless speaker with rich battery life and crisp sound.",
    stockText: "Low stock",
    rating: 2,
    note: "Quick view or add this item to the cart for fast checkout.",
    brand: "Boreal Sound",
  },
  {
    id: "6",
    image:
      "https://images.unsplash.com/photo-1590658268037-6bf12165a8df?w=400&h=300&fit=crop&crop=center",
    alt: "Crescendo IEM Pro 2",
    category: "Accessories",
    title: "Crescendo IEM Pro",
    price: "$199.00",
    subtitle:
      "Professional in-ear monitors tuned for clarity and deep bass response.",
    stockText: "In stock",
    rating: 1,
    note: "Quick view or add this item to the cart for fast checkout.",
    brand: "Crescendo",
  },
  {
    id: "RxogW6M19y8",
    image:
      "https://images.unsplash.com/photo-1505740420928-5e560c06d30e?w=400&h=300&fit=crop&crop=center",
    alt: "Auralite Graphite Over-Ear Test",
    category: "Headphones",
    title: "Auralite Graphite Over-Ear Test new",
    price: "$149.00",
    subtitle:
      "Balanced sound profile with memory foam ear cushions and 30-hour battery life.",
    stockText: "In stock",
    rating: 1,
    note: "Quick view or add this item to the cart for fast checkout.",
    brand: "Auralite",
  },
  {
    id: "FZR04dchaX0",
    image:
      "https://images.unsplash.com/photo-1590658268037-6bf12165a8df?w=400&h=300&fit=crop&crop=center",
    alt: "SonicWave Wireless Earbuds",
    category: "Speakers",
    title: "SonicWave Wireless Earbuds",
    price: "$129.99",
    subtitle:
      "True wireless earbuds with active noise cancellation and 8-hour battery life.",
    stockText: "Only 3 left",
    rating: 0.5,
    note: "Quick view or add this item to the cart for fast checkout.",
    brand: "Boreal Sound",
  },
];
